...
to complex-spread
  ...
  [
    ...
    ;; infect neighbors
      ask link-neighbors
      [ 
        if (( random-float 1 <= prob-infection ) and (count link-neighbors with [infected? = true] > 1))  ;; infect with probability p
        [
...
To re-infect a single individual while keeping the same topology - press REINFECT-ONE. 
...
To try complex contagion, you need to infect two neighboring individuals with REINFECT-TWO. Then apply COMPLEX-SPREAD.
...
Also consider the differing effect of rewiring on simple and complex contagion.
