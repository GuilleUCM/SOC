...
to setup
  ...
  ask links [set color white
    ...
    ask turtles ;;this sets pro turtles to color red + 1 or blue + 1
      [
        ifelse pro? = true 
        [set color red + 1]
        [set color blue + 1]
...
to updateopinion
  ...
  ask one-of turtles [
    ...
    if (proneighbors > antineighbors) [
      if (pro? = false) [set shape "square"]
      set pro? true
      set color red + 1
    ]
    if (proneighbors < antineighbors) [
      if (pro? = true) [set shape "square"]
      set pro? false
      set color blue + 1
     ]
     if (proneighbors = antineighbors) [
...
to do-layout
  repeat 400 [layout-spring turtles links 0.05 15 30]
  display
end
@#$#@#$#@
GRAPHICS-WINDOW
